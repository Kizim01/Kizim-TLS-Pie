...
PROJECT_BUNDLE_DIR="$SCRIPT_DIR/TLS-Pie"
...
if [[ -d "$PROJECT_BUNDLE_DIR" ]]; then
  run_cmd cp -a "$PROJECT_BUNDLE_DIR/." "$TARGET_DIR/"
else
  echo "Warning: bundled project folder not found at $PROJECT_BUNDLE_DIR"
fi
