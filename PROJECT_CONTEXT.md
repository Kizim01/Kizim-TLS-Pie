...
see on the couple that are not hidden?"* (it did not: the share divided by
...
- **Built (`scratchpad\mos\refit56.py`, backup
  ...
  `max_points / (8 x n)`; `_rebuild` encodes with those; `refit(hidden)`
  re-reads, through `load` and the carry, only the shown captures holding
  fewer than 0.9 x min(total, share) points, replaces them in place, and
  names them; `density(voxel, hidden)` takes the list too; `remove`
  re-keys the set; `open_project` clears it; the carry block of `density`
  is now `_carry_over`, shared with `refit`. Page: `hiddenList()`,
  `scheduleRefit()` debounced 1.2 s, `refitNow()` posting `/refit`;
